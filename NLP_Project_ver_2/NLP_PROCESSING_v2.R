...
NLP_PROCESSING <- function(xmldf){
    ...
    xmldf <- gsub('¡°', " ", xmldf) # Å«µû¿ÈÇ¥´Â ÇÑÄ­ ¶³¾î¶ß·ÁÁÜ.
    xmldf <- gsub('¡±', " ", xmldf) # Å«µû¿ÈÇ¥´Â ÇÑÄ­ ¶³¾î¶ß·ÁÁÜ.
...
#»êÀÚºÎ ¿ëµµ.(Çöº´·Â/ °æ°ú¿ä¾à µÑ´Ù º¸¿©ÁÖ±â À§ÇÔ )

search_df_1 <- result_xml_df[result_xml_df$`<MN>`=='°æ°ú¿ä¾à',] # ÅÂ±×, °Ë»ö¾î ÁöÁ¤ ex) <MN>, '¾à¸í'
...
for (i in nrow(search_df_1):1){# µÚ¿¡¼­ºÎÅÍ »èÁ¦ÇØ ÇàÀÌ ¹Ğ·Á¼­ »èÁ¦ µÇÁö ¾Êµµ·Ï Ã³¸®ÇÔ.
